# =============================================================================
# biz2credit — DEV WAF Configuration
# Rules mirroring PROD: ShieldMitigation, Admin-Block, Block-IP, block-git,
# BlockSpecificURL, Block-African-Countries, Block-SouthAmerica,
# BlockSelectedCountries1/2, BotControl, IpReputation, AnonymousIp,
# CommonRuleSet, KnownBadInputs, SQLi, AntiDDoS, BlockExtensions,
# Allow-IPs, RateLimit, AllowCountryUS, Allow-URLs
# =============================================================================
...
key    = "waf-alb-existing/terraform.tfstate"
region = "us-east-1"
...
known_bad_inputs_rule_action_overrides = [
  ...
  { name = "Host_localhost_HEADER",              action = "allow" },
...
# NOTE: Disabled — requires ClientSideActionConfig not yet supported in
# Terraform AWS provider HCL. Re-enable once provider adds support.
...
# Restrict Admin — block /admin, /wp-admin paths
...
# Block Specific URLs
enable_block_specific_urls   = true
...
# Block File Extensions in URI path
enable_block_extensions   = true
...
# Block African Countries (split into 2 rules — AWS max 50 codes per geo rule)
...
south_america_country_codes  = ["AF","AR", "BO", "BR", "CL", "CO", "EC", "GY", "PY", "PE", "SR", "UY", "VE"]

# Block Selected Countries Group 1 (e.g. high-risk regions)
enable_block_selected_countries_1   = true
...
enable_block_selected_countries_2   = true
block_selected_countries_2_priority = 83
selected_country_codes_2 = [
  "PK", "BD", "VN", "ID", "MM"
]

# Allow Country US only (block all non-US traffic)
enable_allow_country_us   = true
allow_country_us_priority = 84

# Allow Specific URLs (bypass other rules — evaluated first)
enable_allow_specific_urls   = true
...
# =============================================================================
allowlist_ips      = []   # Add trusted CIDRs here e.g. ["10.0.0.0/16"]
blocklist_ips      = []   # Add blocked CIDRs here
